...
/**
 * Factory middleware auth untuk route HTTP & upgrade WebSocket.
 * `authOk` menilai satu request; `guard` membungkus handler API sehingga
 * request tak terotentikasi langsung ditolak 401 (Req 9.2/9.3). Dipakai
 * composition root (`app.ts`) dan di-inject ke tiap tabel rute via konteks.
 */
export function createApiGuard(config: AuthConfig): {
  authOk: (req: Request) => boolean;
  guard: <Req extends Request, Res extends Response>(
